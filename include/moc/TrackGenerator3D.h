...
private:
  //用来存储2D链表特征线的link数据，用于在GPU中建立索引
  int* track_2D_chain_link_index;

  //用来存储2D链表特征线的一维数组，与上面的索引建立关系
  Track** track_2D_chainon1D;
...
public:
  ...
  //获得 _first_lz_of_stack
  int*** getFirstLzofStack();
  //获得按链排序的3维2D特征线
  int* getTrack2DChain();
  //获得转化后的track2Dchain
  Track** getTrack2DChainon1D();
  //获得每根轴的轨道数
  int** getnumzarray();
  int** getnumlarray();
  //获得每根轴的轴间距
