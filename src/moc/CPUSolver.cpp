...
void CPUSolver::transferAllInterfaceFluxes() {
  ...
  // _timer->printSplits();
  double time_=_timer->getSplit("Idle time");
  double time_2=_timer->getSplit("Transport Sweep");
  double time_3=_timer->getSplit("Total transfer time");
  //double time_4=_timer->getSplit("Unpacking time");
  
  int segments_count= _track_generator->getNumSegments();

  int RankID, ProcessNum, Length; 
	char ProcessName[MPI_MAX_PROCESSOR_NAME]; 
  MPI_Get_processor_name(ProcessName, &Length); 
int coords[3]={0, 0, 0};
  MPI_Cart_coords(MPI_cart, rank, 3,coords);
}
#else
...
#pragma omp parallel for schedule(static)
  for (long r=0; r < _num_FSRs; r++) {
    for (int e=0; e < _NUM_GROUPS; e++)
    {
      ...
      //_old_scalar_flux(r, e) *= norm_factor;/// 为保持与GPU版本一致，不添加此步
  }
  }
...
void CPUSolver::printLoadBalancingReport() {
  ...
  FP_PRECISION fe=mean_segments/max_segments;
  log_printf(NORMAL, "Min / max / mean number of segments in domains: "
             "%.1e / %.1e / %.1e/ %.4f/", float(min_segments), float(max_segments),
             mean_segments,fe);
  ...
  log_printf(NORMAL, "Percentage of tracks exchanged in X:%.4e/Y:%.4e/Z:%.4e direction: "
             "%.2f / %.2f / %.2f %",tracks_x,tracks_y,tracks_z,
              tracks_x / sum_border_tracks_200, tracks_y
             / sum_border_tracks_200, tracks_z / sum_border_tracks_200);
  if(rank==0)
    {
      int domain_struct[3];
      _geometry->getDomainStructure(domain_struct);
      std::string filename = "./load_data";
      
      /* Write the FSR fluxes file */
      FILE* out;
      out = fopen(filename.c_str(), "a+");
      /* Write k-eff */
      fprintf(out,"%d,%d,%d,",domain_struct[0],domain_struct[1],domain_struct[2]);
      fprintf(out,"%ld,%ld,%.0f,",min_segments,max_segments,mean_segments);
      fprintf(out,"%.0f,%.0f,%.0f\n",tracks_x,tracks_y,tracks_z);
    }
}
#endif
