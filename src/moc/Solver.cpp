...
void Solver::setTrackGenerator(TrackGenerator* track_generator) {
  ...
  if (track_generator_3D != NULL) {
	_tracks_3D = track_generator_3D->get3DTracksArray();													
    _fluxes_per_track = _num_groups;
    _tot_num_tracks = track_generator_3D->getNum3DTracks();
    _tot_num_track_2d = _track_generator->getNum2DTracks();
...
void Solver::computeEigenvalue(int max_iters, residualType res_type) {
  ...
  // /*在这里进行传参*/
  //  TrackGenerator3D* track_generator_3D = dynamic_cast<TrackGenerator3D*>(_track_generator);
  //  OTFonGPU OTF_on_GPU;
  //  OTF_on_GPU.clonedatatoGPU(track_generator_3D);
  //  OTF_on_GPU.runonGPU();
...
//  log_printf(NORMAL, "iter %d time: %1.4E sec", i ,Time);
  }
...
void Solver::printTimerReport() {
  ...
  double Track_Generation_Time = _timer->getSplit("Track Generation Time");
  msg_string = "Track Generation Time";
  msg_string.resize(53, '.');
  log_printf(RESULT, "%s%1.4E sec", msg_string.c_str(), Track_Generation_Time);

  double pre_track_Time = _timer->getSplit("pre track Time");
  msg_string = "Pre track Time";
  msg_string.resize(53, '.');
  log_printf(RESULT, "%s%1.4E sec", msg_string.c_str(), pre_track_Time);
  ...
  double transport_sweep_k = _timer->getSplit("Transport Sweep kernel");
  msg_string = "  Transport Sweep";
  msg_string.resize(53, '.');
  log_printf(RESULT, "%s%1.4E sec", msg_string.c_str(), transport_sweep_k);
  ...
  msg1<<num_integrations;
  msg_string = "num_integrations";
  msg_string.resize(53, '.');
  log_printf(RESULT, "%s%s", msg_string.c_str(),
              msg1.str().c_str());
...
void Solver::dumpFSRFluxes(std::string fname) {
  ...
  for (long r=0; r < _num_FSRs; r++) {
    ...
    fwrite(&z, sizeof(double), 1, out);
      fwrite(&_scalar_flux[r*_num_groups], sizeof(FP_PRECISION), _num_groups, out);
      }
  }
  else
  {
    /* Write k-eff */
    fprintf(out,"_keff %lf  ",_k_eff);
    /* Write number of energy groups */
    fprintf(out,"_num_groups %d   ",_num_groups);

    /* Write number of energy groups */
    fprintf(out,"_num_FSRs%ld   \n",_num_FSRs);
    ...
    for (long r=0; r < _num_FSRs; r++) {
      ...
      fprintf(out,"z %lf    ",z);
      for (int e=0; e < _num_groups; e++)
          fprintf(out,"\n %f ",_scalar_flux[r*_num_groups+e]);
