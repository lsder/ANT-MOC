...
void TrackGenerator::generateTracks() {
  ...
  try {
    ...
    if(_read_segments == true&&_dump_segments==true)
      log_printf(ERROR, "_read_segments or _dump_segments must only choose one");
    /* Initialize the track file directory and read in tracks if they exist */
    //NOTE Useful for 2D simulations, currently broken  now fixed 这里默认有文件直接导入
    if(_read_segments == true||_dump_segments==true)
       initializeTrackFileDirectory(); 
    /* If track file not present, generate segments */
    if (_read_segments == false) {
...
void TrackGenerator::initializeTrackFileDirectory() {
  ...
  if (_read_segments == true) {
    if(!stat(_tracks_filename.c_str(), &buffer))
    {
      ...
      _timer->stopTimer();
      _timer->recordSplit("segmentize Time");
      double tot_time = _timer->getSplit("segmentize Time");
      std::string  msg_string = "Total time to solution";
      msg_string.resize(53, '.');
      log_printf(NORMAL,"Total time to load segmentize Time %s%1.4E sec ",msg_string.c_str(), tot_time);
    } 
    else
    {
        log_printf(ERROR, "Cannot read segmentize, file  not exist");
    }
  }
...
void TrackGenerator::dumpSegmentsToFile() {
  ...
  //std::cout<<geometry_to_string<<std::endl;
 ...
 *        tracking file: true to record, false not to record
 */
void TrackGenerator::setReadSegments(bool read_segments) {
...
void TrackGenerator::printMemoryReport() {
  ...
  log_printf(NORMAL, "Max 2D explicit track storage per domain %.2f MB",
             track_storage / float(1e6));
  log_printf(NORMAL, "Max 2D explicit segment storage per domain %.2f MB",
             max_segment_storage / float(1e6));
}

//获得num数组的指针地址
int* TrackGenerator::getaddrNumX(){
  return _num_x;
}

int* TrackGenerator::getaddrNumY(){
  return _num_y;
}