...
void TrackGenerator3D::initializeTracks() {
  ...
  if (_segment_formation == EXPLICIT_3D){
	...
	/* Initialize the 1D array of Tracks for all 3DTracks */
    initialize3DTracksArray();														
  }						  
...
void TrackGenerator3D::initialize2DTrackChains() {
  ...
  track_2D_chain_link_index = new int[sum];
  track_2D_chainon1D = new Track*[_num_2D_tracks];
  ...
  for (int a=0; a < _num_azim/2; a++) {
    ...
    for (int x=0; x < _num_x[a]; x++) {
      ...
      track_2D_chain_link_index[p] = link_index+1;
      p++;
 ...
 */
Track3D** TrackGenerator3D::get3DTracksArray() {
 ...
 */
void TrackGenerator3D::initialize3DTracksArray() {
  ...
  if (_tracks_3D_array != NULL)
    delete [] _tracks_3D_array;
  long num_3D_tracks = getNumTracks();
  _tracks_3D_array = new Track3D*[num_3D_tracks];
    ...
    for (int i=0; i < _num_x[a] + _num_y[a]; i++) {
      for (int p=0; p < _num_polar; p++) {
        for (int z=0; z < _tracks_per_stack[a][i][p]; z++){
          ...
          Track3D* track = &_tracks_3D[a][i][p][z];
          track->setUid(uid);
...
int*** TrackGenerator3D::getFirstLzofStack(){
  return _first_lz_of_stack;
}

//获得按链表排序的2D特征线
int* TrackGenerator3D::getTrack2DChain(){
  return track_2D_chain_link_index;
}

//获得转化后的track2Dchain
Track** TrackGenerator3D::getTrack2DChainon1D(){
  return track_2D_chainon1D;
}

//获得每根轴的轨道数
int** TrackGenerator3D::getnumzarray(){
  return _num_z;
}

int** TrackGenerator3D::getnumlarray(){
  return _num_l;
}

//获得每根轴的轴间距
double** TrackGenerator3D::getdzeff(){
  return _dz_eff;
}

double** TrackGenerator3D::getdleff(){
  return _dl_eff;
}

double TrackGenerator3D::getxmax(){
  return _x_max;
}

double TrackGenerator3D::getxmin(){
  return _x_min;
}

double TrackGenerator3D::getymax(){
  return _y_max;
}

double TrackGenerator3D::getymin(){
  return _y_min;
}

double TrackGenerator3D::getzmax(){
  return _z_max;
}

double TrackGenerator3D::getzmin(){
  return _z_min;
}

long*** TrackGenerator3D::getcumtracksperstack(){
  return _cum_tracks_per_stack;
}

/**
 * @brief Print the track generation memory report.
 */
void TrackGenerator3D::printMemoryReport() {

  long track_storage = _num_3D_tracks * sizeof(Track);
  long segment_storage = getNumSegments() * sizeof(segment);
  ...
  log_printf(NORMAL, "Max 3D explicit track storage per domain %.2f MB",
             track_storage / float(1e6));
  log_printf(NORMAL, "Max 3D explicit segment storage per domain %.2f MB",
